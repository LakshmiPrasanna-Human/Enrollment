...
namespace EnrollmentStateFullService
{
    internal class OwinCommunicationListener : ICommunicationListener, IHttpControllerActivator
    {
        ...
        private readonly IStudentEnrollmentService serviceInstance;

        public OwinCommunicationListener(StatefulServiceContext serviceContext,
                    IStudentEnrollmentService svc, ServiceEventSource eventSource, string endpointName)
            : this(serviceContext, svc, eventSource, endpointName, null)
        {
        }

        public OwinCommunicationListener(StatefulServiceContext serviceContext,
                    IStudentEnrollmentService svc, ServiceEventSource eventSource, string endpointName, string appRoot)
        {
        ...
        public IHttpController Create(HttpRequestMessage request, HttpControllerDescriptor controllerDescriptor, Type controllerType)
        {
            ...
            ConstructorInfo ci = controllerType.GetConstructor(BindingFlags.Instance | BindingFlags.Public, null, CallingConventions.HasThis, new[] { typeof(IStudentEnrollmentService) }, new ParameterModifier[0]);
            if (null != ci)
            {
